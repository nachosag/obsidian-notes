...
- Con abono Care tenés monitoreo y soporte proactivo. Sin abono, soporte por ticket.
...
- **Seguimiento de oportunidad:** lead → relevamiento → presupuesto → instalación → abono opcional. Software y abono son opcionales: sin abono, entrega + 30 días de garantía y luego soporte por ticket; con abono Care, monitoreo y actualizaciones incluidas.
...
flowchart TD
    ...
    Ingresos --> Abono["Abono Care<br/>Plan único personalizado<br/>base USD 20 + variables<br/>USD 25-60/mes<br/>monitoreo + updates + soporte"]
...
**Lectura:** el cliente elige cómo pagar el equipo (compra o comodato en cuotas), elige si quiere Care (recomendado), elige si quiere hosting en taller, y puede sumar desarrollo a medida. Seguro y telemetría son opcionales del abono.
...
| **Abono Care** (a medida, no hay niveles) | Mensual | USD 25–60/mes (base ~USD 20 + variables por TB/apps/acceso/redundancia) | Mensual. Opcional pero recomendado |
| **Hosting Opción B** | Mensual | +USD 15–25/mes | Solo si elige alojar en taller |
| **Seguro hardware** | Mensual o anual prepago | +USD 5–8/mes o USD 60/año | Opcional del Care |
| **Desarrollo a medida** | Por proyecto | USD 500–3.000 | Post-instalación, como mejora |
| **Soporte por ticket** (sin abono) | Por hora | USD 80/h | Solo si no tiene Care, luego de 30 días de garantía |
...
| | Variables: hardware por proyecto, combustible, comisiones, energía/fibra Opción B | | Abono mensual Care opcional pero recomendado — plan único a medida (base ~USD 20 + variables = USD 25–60/mes) | |
...
- Venta de activos (hardware en 1 pago o cuotas + venta residual), cuota por uso (instalación), suscripción opcional pero recomendada (abono Care a medida: base + variables), hosting opcional (Opción B), proyecto (desarrollo a medida opcional) y ticket sin abono. Incluye seguro opcional y descuento por telemetría. Modelo híbrido que maximiza valor de vida del cliente. El abono es único y a medida, no hay niveles Basic/Pro.
...
### 10.5 Palancas de rentabilidad — cómo ganar más plata
...
**Por qué:** hoy entra ~70% por instalaciones y hardware, y solo ~30% por abonos. El hardware casi no deja ganancia (se compra y se revende); el abono Care deja ~90% de ganancia, porque cuesta poco producirlo (monitoreo + horas).

**Las 5 formas de dar vuelta el número:**

1. **Hacer el abono casi obligatorio.** Si el equipo es nuestro (comodato), el monitoreo protege algo nuestro. Se vende el paquete `comodato + Care 12 meses` con 10% de descuento en el equipo. → El abono pasa del ~30% al 80% de los clientes.

2. **Cobrar la copia de seguridad externa.** La copia en el disco del cliente ya está. Se agrega una copia cifrada (clave del cliente, ver 8.3) en nuestro taller por **+USD 10–15/mes**. → Ingreso mensual nuevo, casi sin costo.

3. **Venderle a empresas, no a particulares.** Un hogar paga USD 30 y discute el precio. Un estudio jurídico de 10 personas paga USD 400/mes hoy. → Menos clientes, más plata por cliente.

4. **Cobrar mantenimiento por las mejoras.** En vez de cobrar una sola vez por un desarrollo a medida, se cobra `USD 800 + USD 20/mes`. → Plata todos los meses en vez de una sola vez.

5. **Cobrar el abono por año.** `USD 25–60/mes → USD 250–550/año` (2 meses de regalo). → Plata por adelantado y el cliente se queda más tiempo.

**Por dónde empezar:** la 1 y la 2 son cambios chicos (ajustar tablas y contrato) y son las que más plata agregan. La 3 es reposicionar el negocio (sección 2.4). La 4 ya tiene base en el módulo 6.2. La 5 se hace junto con la 1.
...
No es obligatorio, pero es recomendado. Es un **plan único a medida** — no hay niveles Basic/Pro. Pagás solo por lo que usás.
...
**Opcionales del Care:** seguro de hardware (+USD 5–8/mes) y descuento por telemetría anónima (-USD 3–5/mes, ver 11.8). Todo se presupuesta en el momento y no cambia sin acuerdo.
